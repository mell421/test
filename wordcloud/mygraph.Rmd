...
# CopyS <- data.frame()
  CopyS <- na.omit(CopyS)
  print("semaine")
  sem <- CopyS %>% filter(occ.ECS...4 >= 1)
  sem2 <- sem %>% arrange(desc(sem$occ.ECS...4))
  barplot(sem$occ.ECS...4, 
          las = 2, 
          names.arg = sem$en.cours,
          col =brewer.pal(8, "Dark2"), 
          main ="Top semaine",
          ylab = "Word frequencies",
          ylim=c(0,max(sem$occ.ECS...4)*1.1))
  box()
  ...
  print("recent")
  rec <- CopyS %>% filter(occ.ECR...2 >= 1)
  rec2 <- rec %>% arrange(desc(rec$occ.ECR...2))
  barplot(rec$occ.ECR...2, 
          las = 2, 
          names.arg = rec$en.cours,
          col =brewer.pal(8, "Dark2"), 
          main ="Top recent",
          ylab = "Word frequencies",
          ylim=c(0,max(rec$occ.ECR...2)*1.1))
  box()
  pie(rec2$occ.ECR...2,label=paste(rec2$en.cours,"",rec2$occ.ECR...2))
  print("tout")
  barplot(CopyS$count, 
          las = 2, 
...
plot(df$episode...4~factor(df$status...5),xlab="status",ylab="Ep",col="#FCD203")

plot(df$saison...3~factor(df$status...5),xlab="status",ylab="Saison",col="#FCD203")

plot(df$episode...4~factor(df$saison...3),xlab="saison",ylab="Ep",col="#FCD203")
```
...
layout(matrix(1:4,2,2))
# print("saison")
barplot(table(df$saison...3
),
        las = 2,
        col =brewer.pal(8, "Dark2"), 
        main ="saison",
        ylab = "nb ep",
        ylim=c(0,max(table(df$saison...3
))*1.1)
        )
box()
# print("episode")
barplot(table(df$episode...4),
        las = 2,
        col =brewer.pal(8, "Dark2"), 
        main ="episode",
        ylab = "nb ep",
        ylim=c(0,max(table(df$episode...4))*1.1)
        )
box()
# print("status")
barplot(table(df$status...5),
        las = 2,
        col =brewer.pal(8, "Dark2"), 
        main ="status",
        ylab = "nb ep",
        ylim=c(0,max(table(df$status...5))*1.1)
        )
box()
...
barplot(table(df$nbCarSem),
        las = 2,
        col =brewer.pal(8, "Dark2"), 
        main ="nb caractere sem",
        ylab = "nb sem with x car",
        ylim=c(0,max(table(df$nbCarSem))*1.1)
        )
box()
# print("nb titre mois")
...
barplot(table(df$nbCarMois),
        las = 2,
        col =brewer.pal(8, "Dark2"), 
        main ="nb caractere mois",
        ylab = "nb mois with x car",
        ylim=c(0,max(table(df$nbCarMois))*1.1)
        )
box()
