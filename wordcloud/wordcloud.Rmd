...
<!-- accueil 10 -->
### accueil
```{r accueil_, echo=FALSE, eval=FALSE}
...
tm <- function(text){
  ...
  wcDouble(dtm_d,forme2,"accueil_4.png",delayW) 
}
...
main <- function(text){
  ...
  # Plot the most frequent words
  barplotSimple(dtm_d[1:20,]$freq,dtm_d[1:20,]$word,"Top 20 test2")
  

  #generate word cloud
  wcSimple(dtm_d$word,dtm_d$freq)
  wcDouble(dtm_d,forme,"test2_3.png",delayW)
}
...
```{r resume, eval=FALSE, include=FALSE}
### resume
text2 <- data.frame(aResume())
text <- text2[2]
main <- function(text){
  dtm_d <- wc(text)
  # Display the top 20 most frequent words
